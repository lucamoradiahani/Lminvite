...
// Banner file auto-detection (banner.gif, banner.png, banner.jpg, banner.webp)
const BANNER_PATHS = [
  path.join(__dirname, "banner.gif"),
  path.join(__dirname, "banner.png"),
  path.join(__dirname, "banner.jpg"),
  path.join(__dirname, "banner.webp"),
];
...
function createLeaderboardEmbed(guild, db) {
  ...
  if (banner) {
    embed.setImage(banner.attachmentUrl);
  }
...
async function updateLiveLeaderboard() {
  ...
  try {
    ...
    const banner = getBannerInfo();
    const files = banner ? [banner.attachment] : [];
...
// ── Helper: Create Interactive Terms & Rules Message (English / Dutch Toggle) ──
function createRulesMessage(lang = "en", guild = null) {
  const isEn = lang === "en";

  const title = isEn
    ? "📜 LM Shop — Terms of Service & Server Rules"
    : "📜 LM Shop — Algemene Voorwaarden & Regels";

  let desc = "";

  if (isEn) {
    desc = 
`### 🛒 General Terms & Warranty Policy
By placing an order, you confirm that you agree to the terms below. Please read them carefully before making a purchase.

---

### 📖 1. Usage & Compliance
• **Discord Terms of Service:** All customers and members must follow Discord's official Terms of Service and Community Guidelines.
• **No Self-Promo / Advertising:** Do **not** promote, advertise, or DM other shops/services in this server unless you have purchased an official paid promotion from LM Shop staff.
• **Fraud & Abuse:** Fraudulent payments, payment abuse, chargebacks, or scamming will result in an immediate and permanent blacklist & ban from our store and Discord.

---

### 📦 2. Products & Warranty
• **Product-Specific Warranty:** Warranty duration and replacement conditions vary per product. There is no universal warranty period for all products.
• **Read Product Information:** Always carefully check the product description, warranty terms, and usage instructions before purchasing.
• **Warranty Void:** The warranty is void if an issue is caused by misuse, changing account settings improperly, or failure to follow provided instructions.

---

### 💳 3. Payments & Sales
• **Digital Goods Policy:** All sales of digital products and services are final. Refunds are not provided once delivered.
• **Check Before Purchasing:** If you are unsure about a product or requirement, open a support ticket before ordering.

---

### 💰 4. Supported Payment Methods
We accept: **iDEAL** • **Tikkie 💸** • **Bancontact** • **PayPal** • **Credit Card** • **Litecoin / LTC (Crypto) ⚡**

---

### 🛠️ 5. Support & Warranty Claims
• For any questions, issues, or replacement claims, please open a support ticket in <#tickets>.
• **Privacy:** Never share order details, account credentials, or transaction IDs in public channels.

⚠️ **Important:** By placing an order or participating in the server, you confirm that you have read and agreed to these terms.`;
  } else {
    desc = 
`### 🛒 Algemene Voorwaarden & Garantiebeleid
Door een bestelling te plaatsen, verklaart u zich akkoord met onderstaande voorwaarden. Lees deze daarom zorgvuldig door vóór uw aankoop.

---

### 📖 1. Gebruik & Naleving
• **Discord Gebruiksvoorwaarden:** Alle klanten en leden dienen zich te houden aan de officiële gebruiksvoorwaarden en richtlijnen van Discord.
• **Geen Zelfpromotie / Reclame:** Het promoten van andere shops of services in chats of via DM's is ten strengste verboden, tenzij er sprake is van een betaalde promo via LM Shop staff.
• **Fraude & Misbruik:** Frauduleuze betalingen, misbruik van betaalmethodes of ongegronde chargebacks leiden tot een onmiddellijke en permanente ban van de winkel en Discord server.

---

### 📦 2. Producten & Garantie
• **Productspecifieke Garantie:** De garantieduur en vervangingsvoorwaarden verschillen per product. Er geldt geen algemene garantieperiode voor alle producten.
• **Lees de Productinformatie:** Controleer vóór aankoop altijd de volledige productbeschrijving, garantievoorwaarden en eventuele gebruiksinstructies.
• **Garantievoorwaarden:** De garantie vervalt wanneer een probleem is ontstaan door verkeerd gebruik of het niet opvolgen van de aangegeven instructies.

---

### 💳 3. Betaling & Verkoop
• **Digitale Producten:** Alle verkopen van digitale producten zijn definitief. Terugbetalingen worden na levering niet aangeboden.
• **Controleer vóór Aankoop:** Twijfelt u over een product? Neem dan vóór het plaatsen van uw bestelling contact op via een support ticket.

---

### 💰 4. Geaccepteerde Betaalmethodes
Wij accepteren: **iDEAL** • **Tikkie 💸** • **Bancontact** • **PayPal** • **Creditcard** • **Litecoin / LTC (Crypto) ⚡**

---

### 🛠️ 5. Support & Garantieclaims
• Voor vragen, problemen of garantieclaims dient u een supportticket te openen in <#tickets>.
• **Privacy:** Deel nooit bestelgegevens, inloggegevens of transactie-informatie in openbare kanalen.

⚠️ **Belangrijk:** Door een bestelling te plaatsen bevestigt u dat u deze voorwaarden hebt gelezen en ermee akkoord gaat.`;
  }
  ...
  const embed = new EmbedBuilder()
    ...
    .setThumbnail(guild?.iconURL({ dynamic: true }) || "https://lmshop.netlify.app/favicon.svg")
    .setFooter({
      text: isEn ? "LM Shop • Click buttons below to switch language • https://lmshop.netlify.app" : "LM Shop • Klik hieronder om van taal te wisselen • https://lmshop.netlify.app",
      iconURL: "https://lmshop.netlify.app/favicon.svg"
    })
    .setTimestamp();
...
client.once("ready", async () => {
  ...
  const commands = [
    ...
    new SlashCommandBuilder()
      .setName("rules")
      .setDescription("Toon de Algemene Voorwaarden & Server Regels met EN/NL taalswitcher"),
    new SlashCommandBuilder()
      .setName("leaderboard")
    ...
    new SlashCommandBuilder()
      .setName("clear")
      .setDescription("Verwijder berichten in een kanaal / Bulk delete messages (Owner/Admin only)")
      .addIntegerOption((option) =>
        option
          .setName("amount")
          .setDescription("Aantal berichten om te verwijderen (1 t/m 100, standaard 100)")
          .setMinValue(1)
          .setMaxValue(100)
          .setRequired(false)
      )
      .addChannelOption((option) =>
        option
          .setName("channel")
          .setDescription("Selecteer het kanaal (standaard dit kanaal)")
          .addChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement)
          .setRequired(false)
      ),
    new SlashCommandBuilder()
      .setName("nuke")
      .setDescription("Wist ALLE berichten in een kanaal door het kanaal te resetten (Owner/Admin only)")
      .addChannelOption((option) =>
        option
          .setName("channel")
          .setDescription("Selecteer het kanaal om te resetten (standaard dit kanaal)")
          .addChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement)
          .setRequired(false)
      ),
    new SlashCommandBuilder()
      .setName("say")
      .setDescription("Laat de bot een bericht sturen in een gekozen kanaal (Owner/Admin only)")
      .addStringOption((option) =>
        option.setName("message").setDescription("Het bericht dat de bot moet sturen").setRequired(true)
      )
      .addChannelOption((option) =>
        option
          .setName("channel")
          .setDescription("Selecteer het kanaal waar het bericht naartoe moet (standaard dit kanaal)")
          .addChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement)
          .setRequired(false)
      ),
    new SlashCommandBuilder()
      .setName("announce")
      .setDescription("Stuur een officiële LM Shop aankondiging/embed in een gekozen kanaal (Owner/Admin only)")
      .addStringOption((option) =>
        option.setName("message").setDescription("De tekst van de aankondiging").setRequired(true)
      )
      .addStringOption((option) =>
        option.setName("title").setDescription("Titel van het bericht (optioneel)").setRequired(false)
      )
      .addChannelOption((option) =>
        option
          .setName("channel")
      ...
      .addAttachmentOption((option) =>
        option
          .setName("image")
          .setDescription("Upload zelf een eigen afbeelding of GIF voor deze aankondiging (optioneel)")
          .setRequired(false)
      )
      .addStringOption((option) =>
        option
          .setName("image_url")
          .setDescription("Of plak een link naar een afbeelding/GIF (optioneel)")
          .setRequired(false)
      )
      .addStringOption((option) =>
        option
          .setName("banner")
          .setDescription("Kies banner weergave (standaard: bot banner.gif)")
          .addChoices(
            { name: "Standaard LM Shop Banner GIF", value: "default" },
            { name: "Geen banner / Geen afbeelding", value: "none" }
          )
          .setRequired(false)
      )
      .addStringOption((option) =>
        option
          .setName("ping")
          .setDescription("Kies een ping (@everyone of @here)")
          .addChoices(
            { name: "Geen ping (None)", value: "none" },
      ...
      .addStringOption((option) =>
        option
          .setName("color")
          .setDescription("Kleur van het kader (optioneel)")
          .addChoices(
            { name: "Cyan Neon (#00F0FF)", value: "#00F0FF" },
            { name: "Discord Blurple (#5865F2)", value: "#5865F2" },
            { name: "Emerald Green (#2ED573)", value: "#2ED573" },
            { name: "Hot Pink (#FF4757)", value: "#FF4757" },
            { name: "Gold (#FFA502)", value: "#FFA502" },
            { name: "Dark Indigo (#141929)", value: "#141929" }
          )
          .setRequired(false)
      ),
    new SlashCommandBuilder()
      .setName("restock")
      .setDescription("Plaats een stijlvolle restock post met Product en Prijs kolommen (Owner/Admin only)")
      .addStringOption((option) =>
        option
          .setName("title")
          .setDescription("Titel van de restock (bijv: YouTube Premium Restocked)")
          .setRequired(true)
      )
      .addStringOption((option) =>
        option
          .setName("product")
          .setDescription("Productnaam of opties (gebruik \\n of enter voor meerdere regels)")
          .setRequired(true)
      )
      .addStringOption((option) =>
        option
          .setName("price")
          .setDescription("Prijs per optie (bijv: €3.50 \\n €13.50)")
          .setRequired(true)
      )
      .addStringOption((option) =>
        option
          .setName("description")
          .setDescription("Korte beschrijving boven de tabel (optioneel)")
          .setRequired(false)
      )
      .addChannelOption((option) =>
        option
      ...
      .addAttachmentOption((option) =>
        option
          .setName("image")
          .setDescription("Upload een afbeelding of GIF (optioneel)")
          .setRequired(false)
      )
      .addStringOption((option) =>
        option
      ...
      .addStringOption((option) =>
        option
          ...
          .setDescription("Kleur van het kader (optioneel)")
          .addChoices(
            { name: "Rood / YouTube Red (#FF0000)", value: "#FF0000" },
            { name: "Cyan Neon (#00F0FF)", value: "#00F0FF" },
            { name: "Discord Blurple (#5865F2)", value: "#5865F2" },
            { name: "Emerald Green (#2ED573)", value: "#2ED573" },
            { name: "Hot Pink (#FF4757)", value: "#FF4757" },
  ...
  try {
    console.log("Synchronizing slash commands...");
    // 1. Wipe duplicate guild-specific commands so nothing is doubled
    for (const [guildId, guild] of client.guilds.cache) {
      try {
        await rest.put(Routes.applicationGuildCommands(client.user.id, guildId), { body: [] });
        console.log(`✓ Cleaned duplicate guild commands for: ${guild.name}`);
      } catch (gErr) {
        // ignore
      }
    }
    // 2. Register single clean set of Global application commands
    await rest.put(Routes.applicationCommands(client.user.id), { body: commands });
    console.log("✓ Slash commands successfully registered cleanly without duplicates!");
  } catch (err) {
...
client.on("interactionCreate", async (interaction) => {
  ...
  // 2. Button Clicks (Claim Member Role, Close Ticket, Still Need Help, or Rules Language Switch)
  if (interaction.isButton()) {
    // 🌐 Rules Language Switcher (English / Nederlands)
    if (interaction.customId === "rules_lang_en" || interaction.customId === "rules_lang_nl") {
      const lang = interaction.customId === "rules_lang_en" ? "en" : "nl";
      const msgData = createRulesMessage(lang, interaction.guild);
  ...
  // /rules (Terms of Service & Rules with Language Switcher)
  if (commandName === "rules") {
    const msgData = createRulesMessage("en", interaction.guild);
    return interaction.reply(msgData);
  }
  ...
  if (commandName === "setleaderboard") {
    ...
    const banner = getBannerInfo();
    const files = banner ? [banner.attachment] : [];
    const sentMsg = await interaction.channel.send({ embeds: [embed], files });
  ...
  if (commandName === "clear") {
    ...
    if (!targetChannel || !targetChannel.isTextBased()) {
      return interaction.reply({ content: "❌ Ongeldig tekstkanaal geselecteerd.", ephemeral: true });
    }
    ...
    try {
      ...
      return interaction.editReply({
        content: `🧹 **${deleted.size} berichten succesvol verwijderd** in ${targetChannel.id === interaction.channelId ? "dit kanaal" : `<#${targetChannel.id}>`}! *(Berichten ouder dan 14 dagen kunnen volgens Discord API regels niet in bulk gewist worden; gebruik hiervoor eventueel \`/nuke\`)*`,
      });
    } catch (err) {
      console.error("Error running clear command:", err);
      return interaction.editReply({
        content: `⚠️ Fout bij het verwijderen van berichten: ${err.message}`,
      });
    }
  }

  // /nuke (Owner & Admins only)
  if (commandName === "nuke") {
    if (!isOwnerOrStar(member)) {
      return interaction.reply({
        content: "❌ Dit commando is alleen beschikbaar voor de **Server Eigenaar** en **Admins**!",
    ...
    const isCurrent = targetChannel.id === interaction.channelId;

    if (!isCurrent) {
      await interaction.reply({
        content: `💣 **Kanaal <#${targetChannel.id}> wordt gereset en gewist...**`,
        ephemeral: true,
      });
    } else {
      await interaction.reply({ content: "💣 **Dit kanaal wordt gereset en alle berichten gewist...**", ephemeral: true }).catch(() => null);
    }
    ...
    try {
      ...
      const newChannel = await targetChannel.clone({
        ...
        parent: parent,
        position: position,
        reason: `Channel nuked/cleared by ${interaction.user.tag}`,
      });

      await targetChannel.delete(`Nuked by ${interaction.user.tag}`).catch(() => null);

      const nukeEmbed = new EmbedBuilder()
        .setTitle("💣 Channel Cleared & Reset")
        .setColor("#FF4757")
        .setDescription(
          `Dit kanaal is volledig leeggemaakt en gereset door <@${interaction.user.id}>! ✨\n\n` +
          `• Alle oude berichten zijn gewist.\n` +
          `• Alle kanaalrechten en instellingen zijn behouden.`
        )
        .setImage("https://media.giphy.com/media/HhTXt43zEX8uQ/giphy.gif")
        .setFooter({ text: "LM Shop Moderation • https://lmshop.netlify.app", iconURL: "https://lmshop.netlify.app/favicon.svg" })
        .setTimestamp();

      await newChannel.send({ embeds: [nukeEmbed] });

      if (db.liveMessage && db.liveMessage.channelId === targetChannel.id) {
        db.liveMessage.channelId = newChannel.id;
        db.liveMessage.messageId = null;
        saveDB(db);
      }
    } catch (err) {
      console.error("Error nuking channel:", err);
      if (!isCurrent) {
        await interaction.followUp({ content: `⚠️ Fout bij het nuken: ${err.message}`, ephemeral: true }).catch(() => null);
      }
    }
    return;
  }

  // /say (Owner & Admins only)
  if (commandName === "say") {
    if (!isOwnerOrStar(member)) {
      return interaction.reply({
        content: "❌ Dit commando is alleen beschikbaar voor de **Server Eigenaar** en **Admins**!",
    ...
    try {
      await targetChannel.send({ content: text.replace(/\\n/g, "\n") });
      return interaction.reply({
        content: `✅ Bericht succesvol verzonden naar <#${targetChannel.id}>!`,
        ephemeral: true,
      });
    } catch (err) {
      console.error("Error running /say command:", err);
      return interaction.reply({
        content: `⚠️ Fout bij verzenden naar <#${targetChannel.id}>: ${err.message}`,
        ephemeral: true,
      });
    }
  }

  // /announce (Owner & Admins only)
  if (commandName === "announce") {
    ...
    const text = interaction.options.getString("message");
    const title = interaction.options.getString("title") || "📢 LM Shop Announcement";
    const targetChannel = interaction.options.getChannel("channel") || interaction.channel;
    const ping = interaction.options.getString("ping") || "none";
    const color = interaction.options.getString("color") || "#00F0FF";
    ...
    const embed = new EmbedBuilder()
      ...
      .setThumbnail(interaction.guild.iconURL({ dynamic: true }) || "https://lmshop.netlify.app/favicon.svg")
      .setFooter({ text: "LM Shop Official • https://lmshop.netlify.app", iconURL: "https://lmshop.netlify.app/favicon.svg" })
      .setTimestamp();

    const customAttachment = interaction.options.getAttachment("image");
    const customImageUrl = interaction.options.getString("image_url");
    const bannerChoice = interaction.options.getString("banner");

    const files = [];

    if (customAttachment) {
      embed.setImage(customAttachment.url);
    } else if (customImageUrl) {
      embed.setImage(customImageUrl.trim());
    } else if (bannerChoice === "none") {
      // User explicitly requested no banner/image
    } else {
      // Default to bot banner.gif if present
      const banner = getBannerInfo();
      if (banner) {
        embed.setImage(banner.attachmentUrl);
        files.push(banner.attachment);
    ...
    try {
      ...
      return interaction.reply({
        content: `✅ Aankondiging succesvol geplaatst in <#${targetChannel.id}>!`,
        ephemeral: true,
      });
    } catch (err) {
      console.error("Error running /announce command:", err);
      return interaction.reply({
        content: `⚠️ Fout bij plaatsen van aankondiging: ${err.message}`,
        ephemeral: true,
      });
    }
  }

  // /restock (Owner & Admins only)
  if (commandName === "restock") {
    ...
    let productInput = interaction.options.getString("product");
    let priceInput = interaction.options.getString("price");
    ...
    let descriptionText = "";
    if (descInput) {
      descriptionText = descInput.replace(/\\n/g, "\n");
    } else {
      const cleanProductName = title.replace(/restock(ed)?/i, "").trim() || "Dit product";
      descriptionText = `**${cleanProductName}** is **back in stock** — secure yours before it sells out again.`;
    }

    const formattedProducts = productInput.replace(/\\n/g, "\n").trim();
    const formattedPrices = priceInput.replace(/\\n/g, "\n").trim();
    ...
    const embed = new EmbedBuilder()
      ...
      .setFooter({ text: "LM Shop • Restock • https://lmshop.netlify.app", iconURL: "https://lmshop.netlify.app/favicon.svg" })
      .setTimestamp();
    ...
    try {
      ...
      return interaction.reply({
        content: `✅ Restock aankondiging succesvol geplaatst in <#${targetChannel.id}>!`,
        ephemeral: true,
      });
    } catch (err) {
      console.error("Error running /restock command:", err);
      return interaction.reply({
        content: `⚠️ Fout bij plaatsen van restock: ${err.message}`,
        ephemeral: true,
      });
    }
  }
...
client.on("messageCreate", async (message) => {
  ...
  // !rules / !regels / !voorwaarden / !tos
  if (content === "!rules" || content === "!regels" || content === "!voorwaarden" || content === "!tos") {
    const msgData = createRulesMessage("en", message.guild);
  ...
  if (content === "!setleaderboard") {
    ...
    const banner = getBannerInfo();
    const files = banner ? [banner.attachment] : [];
    const sentMsg = await message.channel.send({ embeds: [embed], files });
  ...
  // !clear / !purge (Owner / Admin only)
  if (content.startsWith("!clear") || content.startsWith("!purge")) {
    if (!isOwnerOrStar(member)) {
      return message.reply("❌ This command is restricted to the **Server Owner** and **Admins**!");
    }
    ...
    try {
      ...
      const deleted = await targetChannel.bulkDelete(amount, true);
      const confirmMsg = await message.channel.send(`🧹 **${deleted.size} berichten verwijderd** in ${targetChannel.id === message.channel.id ? "dit kanaal" : `<#${targetChannel.id}>`}!`);
      setTimeout(() => confirmMsg.delete().catch(() => null), 4000);
    } catch (err) {
      return message.reply(`⚠️ Fout bij het verwijderen van berichten: ${err.message}`);
    }
    return;
  }

  // !nuke (Owner / Admin only)
  if (content.startsWith("!nuke")) {
    ...
    try {
      ...
      await targetChannel.delete(`Nuked by ${message.author.tag}`).catch(() => null);

      const nukeEmbed = new EmbedBuilder()
        .setTitle("💣 Channel Cleared & Reset")
        .setColor("#FF4757")
        .setDescription(
          `Dit kanaal is volledig leeggemaakt en gereset door <@${message.author.id}>! ✨\n\n` +
          `• Alle oude berichten zijn gewist.\n` +
          `• Rechten en instellingen zijn behouden.`
        )
        .setImage("https://media.giphy.com/media/HhTXt43zEX8uQ/giphy.gif")
        .setFooter({ text: "LM Shop Moderation • https://lmshop.netlify.app", iconURL: "https://lmshop.netlify.app/favicon.svg" })
        .setTimestamp();
      ...
      if (!isCurrent) {
        message.reply(`⚠️ Fout bij het nuken: ${err.message}`).catch(() => null);
      }
    }
    return;
  }

  // !say (Owner / Admin only)
  if (content.startsWith("!say")) {
    ...
    const targetChannel = message.mentions.channels.first() || message.channel;
    let text = message.content.slice(4).trim();

    if (message.mentions.channels.size > 0 && text.startsWith(`<#${targetChannel.id}>`)) {
      text = text.slice(`<#${targetChannel.id}>`.length).trim();
    }

    if (!text) {
      return message.reply("Usage: `!say [#channel] Jouw bericht hier`");
    }
    ...
    try {
      ...
      if (targetChannel.id === message.channel.id) {
        ...
        const confirmMsg = await message.reply(`✅ Bericht verzonden naar <#${targetChannel.id}>!`);
        setTimeout(() => confirmMsg.delete().catch(() => null), 3000);
      }
    } catch (err) {
      return message.reply(`⚠️ Fout bij verzenden: ${err.message}`);
    }
    return;
  }

  // !announce (Owner / Admin only)
  if (content.startsWith("!announce")) {
    ...
    const targetChannel = message.mentions.channels.first() || message.channel;
    let text = message.content.slice(9).trim();
    ...
    if (!text) {
      return message.reply("Usage: `!announce [#channel] Jouw aankondigingstekst hier`");
    }

    const embed = new EmbedBuilder()
      .setTitle("📢 LM Shop Mededeling / Announcement")
      .setColor("#00F0FF")
      .setDescription(text)
      .setThumbnail(message.guild.iconURL({ dynamic: true }) || "https://lmshop.netlify.app/favicon.svg")
      .setFooter({ text: "LM Shop Official • https://lmshop.netlify.app", iconURL: "https://lmshop.netlify.app/favicon.svg" })
      .setTimestamp();
    ...
    try {
      ...
      if (targetChannel.id === message.channel.id) {
        ...
        const confirmMsg = await message.reply(`✅ Aankondiging geplaatst in <#${targetChannel.id}>!`);
        setTimeout(() => confirmMsg.delete().catch(() => null), 3000);
      }
    } catch (err) {
      return message.reply(`⚠️ Fout bij plaatsen van aankondiging: ${err.message}`);
    }
    return;
  }

  // !restock (Owner / Admin only)
  if (content.startsWith("!restock")) {
    ...
    const targetChannel = message.mentions.channels.first() || message.channel;
    let text = message.content.slice(8).trim();

    if (message.mentions.channels.size > 0 && text.startsWith(`<#${targetChannel.id}>`)) {
      text = text.slice(`<#${targetChannel.id}>`.length).trim();
    }

    // Format: !restock Title | Product | Price
    const parts = text.split("|").map((p) => p.trim());
    const title = parts[0] || "YouTube Premium Restocked";
    const product = parts[1] || "YouTube Premium 1m [ INVITE LINK ON OWN ACCOUNT ]\nYouTube Premium 1 Year [ INVITE LINK ON OWN ACCOUNT ]";
    const price = parts[2] || "€3.50\n€13.50";

    const embed = new EmbedBuilder()
      .setTitle(title)
      .setColor("#FF0000")
      .setDescription(`**${title.replace(/restock(ed)?/i, "").trim()}** is **back in stock** — secure yours before it sells out again.`)
      .addFields(
        {
          name: "📦 Product",
          value: `\`\`\`\n${product}\n\`\`\``,
    ...
    try {
      ...
      if (targetChannel.id === message.channel.id) {
        ...
        const confirmMsg = await message.reply(`✅ Restock geplaatst in <#${targetChannel.id}>!`);
        setTimeout(() => confirmMsg.delete().catch(() => null), 3000);
      }
    } catch (err) {
      return message.reply(`⚠️ Fout bij plaatsen van restock: ${err.message}`);
    }
    return;
  }
});
