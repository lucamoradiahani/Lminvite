...
// Save DB (debounced cloud backup)
...
function saveDB(data) {
  ...
  // Auto-backup to Discord Channel Cloud Backup every 5 seconds
...
// ── Cloud Database Backup & Auto-Restore System ──
async function backupDBToDiscord(data) {
  for (const [, guild] of client.guilds.cache) {
    try {
      // Look for a storage channel or log channel
      let channel = guild.channels.cache.find(
        (c) => c.isTextBased() && (c.name.includes("bot-data") || c.name.includes("backup") || c.name.includes("bot-log"))
      );

      if (!channel) {
        // Find any staff / bot / audit channel
        channel = guild.channels.cache.find(
          (c) => c.isTextBased() && (c.name.includes("bot") || c.name.includes("staff") || c.name.includes("admin"))
        );
      }

      if (channel) {
        // Pin or update a backup message
        const messages = await channel.messages.fetch({ limit: 10 }).catch(() => null);
        let backupMsg = messages?.find((m) => m.author.id === client.user.id && m.content.startsWith("<!-- LM_DB_BACKUP_V1 -->"));

        const payload = "<!-- LM_DB_BACKUP_V1 -->```json\n" + JSON.stringify(data) + "\n```";
        if (payload.length < 2000) {
          if (backupMsg) {
            await backupMsg.edit(payload).catch(() => null);
          } else {
...
async function restoreDBFromCloud(guild) {
  ...
  try {
    ...
    for (const [, channel] of channels) {
      ...
      const backupMsg = messages.find((m) => m.author.id === client.user.id && m.content.startsWith("<!-- LM_DB_BACKUP_V1 -->"));
      if (backupMsg) {
        const jsonMatch = backupMsg.content.match(/```json\n([\s\S]*?)\n```/);
        if (jsonMatch && jsonMatch[1]) {
          ...
          if (cloudData.users) {
            ...
            saveDB(db);
            console.log("✓ Successfully restored and merged DB from Discord Cloud Backup!");
            return true;
          }
        }
      }
...
client.once("ready", async () => {
  ...
  // 1. Recover Invites & Synchronize with Discord API on Startup
  for (const [guildId, guild] of client.guilds.cache) {
    try {
      // First try to restore bonus & existing stats from Discord cloud backup
      ...
      // Backfill / sync actual invite uses from Discord's official server invites
      for (const [inviterId, uses] of Object.entries(inviterCounts)) {
        if (!db.users[inviterId]) {
          ...
          // ensure regular count is at least the live invite links sum
          db.users[inviterId].regular = Math.max(db.users[inviterId].regular || 0, uses);
        }
      }
      saveDB(db);

      console.log(`✓ Discord Invites Synced & Restored for: ${guild.name} (${firstInvites.size} active invite links)`);

      // Auto-reconnect live message if needed
      if (db.liveMessage && db.liveMessage.channelId && db.liveMessage.messageId) {
        updateLiveLeaderboard();
      } else {
        // Search if a leaderboard message was already posted in this guild
  ...
  // Register Slash Commands in English
  const commands = [
    new SlashCommandBuilder()
      .setName("member")
      .setDescription("Create a verification role message for members to click and claim Member role (Owner/Admin only)"),
    new SlashCommandBuilder()
      .setName("done")
      .setDescription("Mark this ticket as completed, request customer review, and auto-close (Owner/Admin only)"),
    new SlashCommandBuilder()
      .setName("leaderboard")
      .setDescription("View the Top 10 Monthly Invite Leaderboard and prizes"),
    new SlashCommandBuilder()
      .setName("setleaderboard")
      .setDescription("Post a live auto-updating leaderboard in this channel (Owner/Admin only)"),
    new SlashCommandBuilder()
      .setName("invites")
      .setDescription("Check your invite statistics (Members, Boosters and Customers)")
      .addUserOption((option) =>
        option.setName("user").setDescription("The user whose invites you want to check").setRequired(false)
      ),
    new SlashCommandBuilder()
      .setName("addinvites")
      .setDescription("Add bonus invites to a member (Owner/Admin only)")
      .addUserOption((option) => option.setName("user").setDescription("The member").setRequired(true))
      .addIntegerOption((option) => option.setName("amount").setDescription("Number of bonus invites").setRequired(true)),
    new SlashCommandBuilder()
      .setName("resetinvites")
      .setDescription("Reset all invite statistics for a new month/round (Owner/Admin only)"),
  ].map((cmd) => cmd.toJSON());
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (commandName === "member") {
    ...
    return interaction.reply({
      content: "✓ **Member verification message created!** Members can now click the button to claim the Member role.",
      ephemeral: true,
    });
  }
