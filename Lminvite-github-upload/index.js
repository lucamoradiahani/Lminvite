  ...
  for (const [channelId, info] of Object.entries(db.completedTickets)) {
    if (now >= info.autoCloseAt) {
      try {
        ...
        if (channel) {
          await channel.send("⏳ **24 hours have passed without a review.** Generating transcript and closing ticket...").catch(() => null);
...
client.once("ready", async () => {
  ...
  const commands = [
    new SlashCommandBuilder()
      .setName("close")
      .setDescription("Close this ticket, generate chat transcript, and send to customer DM & mod-logs (Staff only)"),
    new SlashCommandBuilder()
      .setName("member")
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (commandName === "close") {
    if (!isOwnerOrStar(member)) {
      return interaction.reply({
        content: "❌ This command is restricted to the **Server Owner** and **Staff**!",
        ephemeral: true,
      });
    }

    await interaction.reply("📁 **Closing ticket...** Generating chat transcript and sending directly to the customer's DM and staff logs!");
