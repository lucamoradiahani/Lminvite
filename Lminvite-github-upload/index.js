...
client.once("ready", async () => {
  ...
  try {
    ...
    // 1. Register directly per guild for instant 0-second sync
    for (const [guildId, guild] of client.guilds.cache) {
      try {
        await rest.put(Routes.applicationGuildCommands(client.user.id, guildId), { body: commands });
        console.log(`✓ Instant slash commands registered for guild: ${guild.name}`);
      } catch (gErr) {
        console.warn(`Could not register guild commands for ${guild.name}:`, gErr.message);
      }
    }
    // 2. Also register globally
    await rest.put(Routes.applicationCommands(client.user.id), { body: commands });
    console.log("✓ Slash commands successfully registered globally & per guild!");
  } catch (err) {
